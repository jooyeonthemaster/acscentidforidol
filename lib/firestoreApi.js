import {
  ...
  RecipeModel,
  ConfirmationModel
  // BatchOperations - 마이그레이션 함수 주석 처리로 미사용
...
export const getSessionsOptimized = async (page = 1, pageSize = 10, statusFilter = null) => {
  try {
    ...
    // 🚀 커서 기반 페이지네이션 사용 (오프셋 기반 대신)
    let lastDoc = null;
    
    // 첫 페이지가 아닌 경우, 이전 페이지의 마지막 문서를 찾기
    if (page > 1) {
      // 🔧 개선: 캐시된 커서 사용 (실제 구현 시 Redis 등 활용)
      const cursorCacheKey = `cursor_${page-1}_${pageSize}_${statusFilter || 'all'}`;
      const { getFromCache } = await import('./cacheManager');
      lastDoc = getFromCache(cursorCacheKey, 60000); // 1분 캐시
      
      if (!lastDoc) {
        console.log('커서 캐시 미스, 스킵 쿼리 실행...');
    ...
    // 🔄 병렬로 세션 데이터와 총 카운트 조회
    const [result, totalSessions] = await Promise.all([
      SessionModel.getAllPaginated(pageSize, lastDoc, statusFilter),
      getTotalSessionCount()
    ]);
    
    // 다음 페이지용 커서 캐시 저장
    if (result.lastDoc) {
      const cursorCacheKey = `cursor_${page}_${pageSize}_${statusFilter || 'all'}`;
      const { setCache } = await import('./cacheManager');
      setCache(cursorCacheKey, result.lastDoc, 60000); // 1분 캐시
    }
    
    const totalPages = Math.ceil(totalSessions / pageSize);
    
    console.log(`✅ Firestore 세션 조회 완료 - ${result.sessions.length}개 반환 (총 ${totalSessions}개)`);
...
const getTotalSessionCount = async () => {
  ...
  // ✅ 최적화: getCountFromServer 사용 (Firebase 9.6+)
...
// 데이터 마이그레이션 API
// ⚠️ 사용하지 않는 기능 - firebaseApi.js 파일이 없어서 주석 처리
/*
...
const getTotalCount = async (collectionName) => {
  // ✅ 최적화: 전체 문서 다운로드 대신 개수만 조회
  // Before: 200개 문서 = 200 reads
  // After: 200개 문서 = 1 read (99.5% 절감)
